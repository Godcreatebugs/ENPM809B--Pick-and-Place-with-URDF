...
public:
    ...
    const osrf_gear::LogicalCameraImage::ConstPtr & image_msg)
  {
    ...
    // Transformation from to logical_camera_4's frame to world frame.
    // for nine parts

    for(int i=1; i<10; i++) {
    	std::string part_frame = "logical_camera_4_piston_rod_part_";
    	part_frame.append(std::to_string(i));
    	part_frame.append("_frame");
    	//std::cout << "Part Frame: " << part_frame << std::endl; 
    	geometry_msgs::TransformStamped transformStamped;
	    try{
	      transformStamped = tfBuffer.lookupTransform("world", part_frame,
	                               ros::Time(0), timeout);
	    }
	    ...
	    std::cout << part_frame << " in the world frame is: " << std::endl; 
	    ROS_INFO("[%f, %f, %f] [%f, %f, %f] \n",part_frame, transformStamped.transform.translation.x,
    	...
    	///Transformation from logical_camera_1_gasket_part's to world frame.
    	/// For 6 parts
    for(int i=1; i<7; i++) {
    	std::string part_frame = "logical_camera_1_gasket_part_";
    	part_frame.append(std::to_string(i));
    	part_frame.append("_frame");
    	//std::cout << "Part Frame: " << part_frame << std::endl; 
    	geometry_msgs::TransformStamped transformStamped;
